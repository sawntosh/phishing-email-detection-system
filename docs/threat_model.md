...
| 13 | Model | Repudiation of model integrity | Swapped/poisoned `model.json` / `model_text.json` loaded silently | Low | High | **Low** | Both model files are plain JSON (not pickle — avoids arbitrary deserialisation/RCE); the structured model is schema-checked against `FEATURE_NAMES`, the text model against its format version and array lengths; mismatch raises rather than silently loading | `analysis/ml_classifier.py::load`, `analysis/text_model.py::load` |
| 14 | Email ↔ App ↔ Internet | Spoofing / SSRF | An email carries a link crafted to make the server call an internal address (cloud metadata, localhost, LAN) when a lookup is performed | Low (resolver is off by default and analyst-triggered) | High | **Low** | Ingestion makes **no** network calls. Redirect resolution refuses any hop whose DNS answers are not globally routable (private, loopback, link-local, CGNAT, IPv4-mapped IPv6), connects to the *validated* IP (no DNS-rebinding gap), allows only http/https on ports 80/443, HEAD only, 5 hops / 5 s, no body read | `analysis/threat_intel.py::resolve_redirect_chain` |
| 15 | Email ↔ App ↔ Internet | Tampering | Attacker-controlled text steers the VirusTotal request (path/host injection, API-key exfiltration) | Low | Medium | **Low** | Host is a constant; only a strictly validated domain (`DOMAIN_RE`) or SHA-256 hex digest is placed in the path; the HTTP helper refuses any URL not under the VirusTotal API base; key comes from `.env`, never the UI or logs | `analysis/threat_intel.py::lookup_domain`, `::lookup_file_hash` |
| 16 | Browser ↔ App | Repudiation / DoS | Abuse of external lookups to burn API quota or use the app as a scanning proxy | Medium | Low | **Low** | Analyst/admin only, per-user limit of 20 lookups/hour, hard cap of `THREAT_INTEL_MAX_LOOKUPS` per click, every lookup written to the hash-chained audit log | `detector/routes.py::run_threat_intel` |
| 17 | Email ↔ Analyst | Tampering (client side) | Spreadsheet formula injection: a subject/sender such as `=HYPERLINK(...)` executes when an analyst opens a CSV export | Medium | Medium | **Low** | Every CSV cell starting with `= + - @ TAB CR` is prefixed with `'` | `detector/routes.py::_csv_safe` |
| 18 | Email ↔ Analyst | Tampering / DoS | Markup in the subject/body breaks or hijacks the PDF renderer (reportlab parses XML-like tags) | Medium | Low | **Low** | All attacker-controlled text is XML-escaped before it reaches a `Paragraph` | `detector/pdf_report.py::_e` |
| 19 | Email ↔ Analyst | Spoofing (social engineering) | An analyst clicks a live phishing link shown in a result page | Medium | High | **Medium** | Domains and URLs are displayed defanged (`hxxp`, `[.]`), auto-escaped, and never rendered as links (residual: an analyst can still copy and open a defanged link by hand) | `analysis/explanations.py::defang`, `templates/detector/result.html` |
| 20 | Model | Denial of Service | An enormous body makes the text tokenizer consume unbounded CPU | Medium | Medium | **Low** | Input is truncated to 20 000 characters before tokenisation | `analysis/text_model.py::MAX_INPUT_CHARS` |

**Priority order for remediation effort (highest residual risk first):**
Row 12 (adversarial ML evasion) > Rows 1, 4, 6, 7, 8, 19 (Medium) > all remaining
