...
def test_cross_domain_open_redirect_is_flagged_with_target():
    ...
    assert "open_redirect_param" in r["findings"]
...
def test_same_domain_redirect_is_not_flagged():
    ...
    assert "open_redirect_param" not in r["findings"]
...
def test_embedded_url_in_path_is_flagged():
    r = analyse_url("https://good.example.com/redirect/http://evil.top/x")
    assert "embedded_url_in_path" in r["findings"]
